...
def test_post_device(client):
    response = client.post(
        DEVICES_URL, data={"token": (BytesIO(b"foo=bar"), "token.tar.gz")}, content_type="multipart/form-data"
    )
...
def test_post_device_missing_file(client):
    response = client.post(DEVICES_URL, content_type="multipart/form-data")
...
@mock_backend_response({"subordinates": {"set_enabled": {"result": True}, "update_sub": {"result": True}}})
def test_patch_device(client):
    response = client.patch(DEVICE_URL, json={"enabled": False, "options": {"custom_name": "foobar"}})
    assert response.status_code == HTTPStatus.NO_CONTENT


@pytest.mark.parametrize("request_data", [{"enabled": True}, {"options": {"custom_name": "foobar"}}])
@mock_backend_response({"subordinates": {"set_enabled": {"result": True}, "update_sub": {"result": True}}})
