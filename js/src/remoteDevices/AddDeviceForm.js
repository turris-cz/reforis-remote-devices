...
import {
    useAPIPost, API_STATE, useForm, useAlert, FileInput, Button, formFieldsSize,
} from "foris";
...
export default function AddDeviceForm() {
    ...
    return (
        <div className={formFieldsSize}>
            <h3>{_("Add Device")}</h3>
            <p>{_("Upload token file generated by device you want to control remotely.")}</p>
            <form onSubmit={handleSubmit} className="position-relative">
                <FileInput
                    label={formData.token ? formData.token.name : _("Token file")}
                    files={[formData.token]}
                    error={formErrors.token}
                    onChange={formChangeHandler((value) => ({ token: { $set: value } }))}
                    accept=".tar.gz,.tgz"
                />
                <div className="text-right">
                    <Button
                        type="submit"
                        forisFormSize
                        disabled={formErrors.token || !formData.token || isSending}
                        loading={isSending}
                    >
                        {_("Upload token")}
                    </Button>
...
function validator(formData) {
    ...
    if (filename.length === 0 || filename.length > 50) {
        return { token: _("Filename must be at least 1 and at most 50 characters long.") };
    }
    if (!/^[a-zA-Z0-9_.-]*$/.test(filename)) {
        return { token: _("Filename can contain only alphanumeric characters, dots, dashes and underscores.") };
    }
